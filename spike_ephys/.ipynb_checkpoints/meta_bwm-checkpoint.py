...
def plot_bar_neuron_count(table_only=False):
    ...
    #file_ = download_aggregate_tables(one)
    file_ = ('/moto/stats/users/hy2562/projects/ephys_atlas/clusters.pqt')#('/moto/stats/users/hy2562/projects/ephys_atlas/bwm/clusters.pqt')#
    df = pd.read_parquet(file_)
    dfa, palette = get_allen_info()
    ...
    ps = list(set(['_'.join(x) for x in zip(df['Beryl'],df.pid.values)]))    
    n_ins = Counter([x.split('_')[0] for x in ps])
    
    #
    cs = list(set(['_'.join(x) for x in zip(df['Beryl'],str(df['channels'].values))]))    
    c_ins = Counter([x.split('_')[0] for x in cs])
    ...
    nclus_nins = [[c0[reg], n_ins[reg],c_good[reg], c_ins[reg]] for reg in regs1]
    ...
    print(f'{len(regs1)} regions, {sum(nclus_nins[:,2])} neurons,'
    ...
    if table_only:
        ...
        df.to_csv('/moto/stats/users/hy2562/projects/ephys_atlas/table.csv')
    ...
    for k, k0 in zip(range(3), reversed(range(3))):
        ...
        # ax[k].barh(range(len(regs1)), nclus_nins[:,2],color=cols, 
                   # height=barwidth)
        ax[k].barh(range(len(regs1)), nclus_nins[:,3],color=cols, 
           height=barwidth)
        ax[k].set_xscale("log")
...
#         ax[k].vlines(0.1 * nclus_nins[:,1], y_start, y_end, 
#                   color='k', linewidth =2)                     
        ...
        ax2.set_yticklabels(nclus_nins[:,3],fontsize=fs)
...
def neuron_number_swansons():
    ...
    3 Swansons: all neurons, only good neurons,
               recordings
    '''             
    df = pd.read_csv('/moto/stats/users/hy2562/projects/ephys_atlas/table.csv')
    ...
    for c in range(3):
        ...
        norm = mpl.colors.Normalize(min(amps), 
                                    max(amps))
