...
# %%
def denoise_with_phase_shift_no_supress(chan_wfs, phase_shift, chan_ci_idx, spk_sign):
...
#             denoised_im_wfs[0:int(121 - ph),j] = spk_denoised_wfs[int(ph):None, j]
    ...
    # plt.savefig('/moto/stats/users/hy2562/projects/ephys_atlas/phase-shift_single_channel_denoiser_large_scale_test/'+ 'unit_' + str(i) + 'iterative_phase_shift.png')
    # plt.close()
    ...
    while len(q)>0:
        ...
        for k in v:
            if CH_checked[k] == 0:
                ...
                CH_phase_shift[k] = denoise_with_phase_shift_no_supress(full_wfs[:,k], int(parent_peak_phase[k]), k, spk_sign)
                parent[k] = checked_neighbors[phase_shift_ref]
                wfs_ptp[k] = np.ptp(spk_denoised_wfs[:,k])
                q.insert(0,k)
                CH_checked[k] = 1
...
# %%
np.shape(wfs_traveler[0, :, 15][np.newaxis :, np.newaxis])
